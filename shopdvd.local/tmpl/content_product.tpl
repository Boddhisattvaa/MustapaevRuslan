...
<table id="product">
	<tr>
		...
		<td class="product_desc">
			...
			<p>Год выпуска: <span><?=$this->product["year"]?></span></p>
			<p>Вид: <span><?=$this->product["section"]?></span></p>
			<p>Страна-производитель: <span><?=$this->product["country"]?></span></p>
			<p>Фирма: <span><?=$this->product["director"]?></span></p>
			<p>Срок годность: <span><?=$this->product["play"]?></span></p>
			<p>Примечание: <span><?=$this->product["cast"]?></span></p>
			<table>
				<tr>
					<td>
						<p class="price"><?=$this->product["price"]?> сом </p>
					</td>
					<td>
						<p>
			...
			<?php for ($i = 0; $i < count($this->products); $i++) { ?>
				<td>
					<div class="intro_product">
						...
						<p class="price"><?=$this->products[$i]["price"]?> сом </p>
						<p>
